...
## 📊 병원급 이원화(Two-Track) 수가 전략 자동화
이 스크립트를 Antigravity Agent(예: `hira-npay` SKILL)와 연동하면, 추출된 지역별(서울/경기/전국) 가격 통계를 기반으로 **전문적인 HTML 전략 보고서**를 자동으로 생성할 수 있습니다.
- **Track A (AI 통계 기반가)**: 실비보험 삭감 및 환자 거부감을 방어하는 최저/중위 평균가 (기계적 방어 라인)
- **Track B (프리미엄 시그니처 수가)**: 무결점 진료 원칙하에 설정된 합리적 범위 내 최고 가치 (적극 권장 라인)

해당 이원화 프레임워트를 적용하면, 의료 품질을 타협하지 않으면서도 경영진 설득력과 데이터 방어력을 모두 갖춘 프리미엄 수가(Pricing) 리포트가 도출됩니다.
